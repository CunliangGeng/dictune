...
import type { ComparisonResult, DiffPart, LangCode } from "./types";
...
export function lcsArrays(a: string[], b: string[]): RawDiffPart[] {
  ...
  while (i > 0 || j > 0) {
    if (i > 0 && j > 0 && a[i - 1] === b[j - 1]) {
      ops.push({ type: "equal", tok: a[i - 1] });
      i--;
      j--;
    } else if (j > 0 && (i === 0 || dp[i][j - 1] >= dp[i - 1][j])) {
      ops.push({ type: "added", tok: b[j - 1] });
      j--;
    } else {
      ops.push({ type: "removed", tok: a[i - 1] });
      i--;
    }
  }
  ops.reverse();
    ...
    if (part.type === "removed") {
      ...
      if (next && next.type === "added") {
        ...
        if (pairs > 0)
          result.push({
            type: "wrong",
            original: part.value.slice(0, pairs),
            transcribed: next.value.slice(0, pairs),
          });
        if (part.value.length > pairs)
          result.push({ type: "removed", value: part.value.slice(pairs) });
        if (next.value.length > pairs)
          result.push({ type: "added", value: next.value.slice(pairs) });
        k++;
...
export function getPuncSpacePositions(originalText: string): Set<number> {
  ...
  for (const ch of originalText) {
    if (/[\p{P}]/u.test(ch)) {
      if (charIdx >= 0) positions.add(charIdx);
    } else if (ch.trim()) charIdx++;
  }
  return positions;
}

export function insertPuncSpaces(
  diff: DiffPart[],
  ...
  for (const part of diff) {
    if (part.type === "added" || part.type === "space") {
      result.push(part);
      continue;
    }
